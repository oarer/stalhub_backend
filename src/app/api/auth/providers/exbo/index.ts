	...
	.group('/exbo', (app) =>
		app
			...
			.get('/callback', async ({ query, headers, cookie: {refresh_token, access_token}, jwt, set }) => {
				...
				if (!linkUserId) {
					const storedState = await prisma.eXBOAuthState.findUnique({
						where: { state },
					})
				...
				const tokenRes = await fetch('https://exbo.net/oauth/token', {
					method: 'POST',
					headers: {
						'Content-Type':
				...
				try {
					...
					set.status = 400
					return { error: 'Invalid token response', body: tokenText }
				}

				const userRes = await fetch(
					'https://exbo.net/oauth/user',
					{
						headers: {
				...
				if (linkUserId) {
					...
					if (existing) {
						set.status = 409
						return { error: 'This EXBO account is already linked to another user' }
					}
					...
					await prisma.eXBOAuth.create({
						data: {
							...
							refresh_expires_at: tokenData.refresh_expires_in
								? new Date(
									Date.now() +
									tokenData.refresh_expires_in * 1000,
								)
								: null,
							userid: linkUserId,
						},
			...
			.delete('/link', async ({ store }) => {
				const { userId } = fromStore(store)
				await prisma.eXBOAuth.deleteMany({ where: { userid: userId } })
				return { success: true }
			},
				{
					beforeHandle: [requireAuth],

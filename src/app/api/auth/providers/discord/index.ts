...
export const discordAuth = createElysia()
	...
	.group('/discord', (app) =>
		app
			...
			.get('/callback', async ({ query, headers, cookie: { access_token, refresh_token }, jwt, set }) => {
				const params = query as Record<string, string | undefined>
				const code = params.code
				const linkUserId = params.state ? consumeLinkState(params.state) : null
				if (!code) {
					set.status = 400
			...
			.delete('/link', async ({ store }) => {
				const { userId } = fromStore(store)
				await prisma.discordAuth.deleteMany({ where: { userid: userId } })
				return { success: true }
			},
				{
					beforeHandle: [requireAuth],

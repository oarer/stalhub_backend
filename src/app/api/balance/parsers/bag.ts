...
export function parseBag(
	data: ItemData,
	lang = 'ru'
): Record<string, string | number | null> {
	const infoBlocks = data.infoBlocks
	const registry = loadStatsRegistry('bags')
...
export function getBagComparisonList(
	oldData: ItemData,
	newData: ItemData,
	lang = 'ru'
): StatChange[] {
	const lines: StatChange[] = []
	const oldStats = parseBag(oldData, lang)
	const newStats = parseBag(newData, lang)
